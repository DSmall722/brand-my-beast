...
**Now:** 16.0e `WaitlistForm` renders the checkbox. POST body includes it. Playwright: checkbox + submit still 201; pledged on `/` stays `$0`.
**Last merged:** 16.0d `POST /api/waitlist` accepts optional `wantWholeTruck`.
...
- [x] 16.0d `POST /api/waitlist` accepts optional `wantWholeTruck`. Invalid email still 400. Created/exists still 201. Field does not flow into `loadBoardIntentStats`.
