...
**Now:** 16.0d `POST /api/waitlist` accepts optional `wantWholeTruck`. Invalid email still 400. Created/exists still 201. Field does not flow into `loadBoardIntentStats`.
**Last merged:** 16.0c Drizzle: `waitlist_signups.want_whole_truck` boolean default false.
...
- [x] 16.0c Drizzle: `waitlist_signups.want_whole_truck` boolean default false. Memory store in CI gets the same field. No pledged math change.
