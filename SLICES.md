...
**Now:** 18.5 `/llms.txt`: delete the `CLOSE_AT is null` line. Keep `Floor $58,000. Buyout $120,000.` and `No close date.` and the 1–12 list. Verify: body has those money lines, no `CLOSE_AT`, no `null`, no `lease`.
**Last merged:** 18.4 `/waitlist/confirm` with no token: buyer empty state (link missing or expired), not `Missing confirm token.` Back to the board. Verify: `/waitlist/confirm` 200, no `Missing confirm token.`, no env keys, no `lease`.
...
- [x] 18.4 `/waitlist/confirm` with no token: buyer empty state (link missing or expired), not `Missing confirm token.` Back to the board. Verify: `/waitlist/confirm` 200, no `Missing confirm token.`, no env keys, no `lease`.
