...
**Now:** 18.6 Seat hometown lead: drop `48-state`. Keep the circuit names. No hometown-lane restyle (17 lock). Verify: `/panels/hood` and `/panels/driver-door` HTML have no `48-state`; floor `$58,000`; etch still locked under `$120,000`.
**Last merged:** 18.5 `/llms.txt`: delete the `CLOSE_AT is null` line. Keep `Floor $58,000. Buyout $120,000.` and `No close date.` and the 1–12 list. Verify: body has those money lines, no `CLOSE_AT`, no `null`, no `lease`.
...
- [x] 18.5 `/llms.txt`: delete the `CLOSE_AT is null` line. Keep `Floor $58,000. Buyout $120,000.` and `No close date.` and the 1–12 list. Verify: body has those money lines, no `CLOSE_AT`, no `null`, no `lease`.
