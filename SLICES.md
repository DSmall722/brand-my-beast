...
**Now:** 6.12 One ObsidianUI block on the hero primary CTA only.
**Last merged:** 6.11 (#101 Playwright: homepage matches PUBLIC_COPY H1 and Notify me.)
...
- [ ] 6.15 Vercel main-only deploys. New root `vercel.json` with `git.deploymentEnabled` `"*": false`, `"main": true`. Feature branches must not create previews. `main` still auto-deploys. Do not change PUBLIC_COPY, hero, waitlist, panels, operator, intent, Stripe, or nameservers. Playwright still: H1 from 0.9, `Notify me`, no lease, no personal handle. Title: `slice(6.15): vercel main-only deploys`.
