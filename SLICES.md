...
- [x] 18.2 `/terms`: drop `Close date is unset` and `(null)`. Use the PUBLIC_COPY “when seats open / no date” line only. Do not print `CLOSE_AT`. Floor `{formatUsd(FLOOR_USD)}` stays. Verify: `/terms` has `$58,000`, no `null`, no `CLOSE_AT`, no `lease`.
